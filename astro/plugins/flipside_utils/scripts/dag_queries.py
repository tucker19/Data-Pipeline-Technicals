"""
Queries to be ran against Flipside API. Each dict has a name of query to connect with create queries, a desintation table and query.
Number of Transactions per Month, last 6 month(s) is commented out due to using up a lot of query seconds and do not want to be rate lmited
"""
QUERIES = [
  {
    'name': 'Top 10 Senders by amount in last hour',
    'destination_table': 'top_10_senders_amount_hour',
    'query':
      """
  ...
  select day_hour_ts, sender, total_transferred
  from senders_ranked
  where per_hour_transfer_rank <= 10
  order by day_hour_ts, total_transferred DESC;
  """
  },
  {
    'name': 'Top 10 Receiver by amount in last hour',
    'destination_table': 'top_10_receivers_amount_hour',
    'query':
      """
...
select day_hour_ts, receiver, total_transferred
...
"""
  ...
  {
    'name': 'Top 10 Senders for a block_id by transfers',
    'destination_table': 'top_10_senders_by_block_id_day',
    'query':
      """
  ...
  },
  {
    ...
    'destination_table': 'top_10_receivers_by_block_id_day',
    'query':
      """
 -- Top 10 Receivers for a block_id by transfers
 ...
 """
  },
  {
    ...
    'destination_table': 'transactions_per_day',
    'query':
      """
 -- Number of Transactions per Day over last week
 select DATE_TRUNC('day', BLOCK_TIMESTAMP) AS day_ts, count(*) as total_transactions, sum(amount) as total_amount
 from solana.core.fact_transfers
 ...
 """
  ...
  #     'destination_table': 'transactions_per_month',
  #     'query':
  #     """
  #    -- Number of Transactions per Month, last 6 month(s)
  ...
  #    select month_ts, -- count(DISTINCT day) as num_of_days,
